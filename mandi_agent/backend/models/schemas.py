...
class FarmerProfile(BaseModel):
    ...
    fpo_id: Optional[str] = Field(None, description="Farmer Producer Organization ID")
    ...
    category: Optional[str] = Field(None, description="Farmer category (general/sc/st/obc)")
    irrigation_type: Optional[str] = Field(None, description="Irrigation type: rainfed/canal/drip/borewell")
...
class MandiPrice(BaseModel):
    ...
    arrival_tonnes: Optional[float] = Field(None, ge=0, description="Arrival quantity in tonnes")
    ...
    def validate_price_consistency(self) -> "MandiPrice":
        ...
        if not (self.min_price <= self.modal_price <= self.max_price):
            raise ValueError(f"modal_price must be between min_price and max_price")
        return self
...
class FarmerAdvisory(BaseModel):
    ...
    target_mandi: Optional[str] = Field(None, description="Recommended mandi if applicable")
    ...
    bundle_saving: Optional[float] = Field(None, ge=0, description="Bundle transport saving")
...
class GuardrailResult(BaseModel):
    ...
    override_reason: Optional[str] = Field(None, description="Override reason if flagged")
...
class VoiceSession(BaseModel):
    ...
    farmer_id: str = Field(..., min_length=1, description="Farmer identifier")
    input_audio_url: Optional[str] = Field(None, description="Input audio file URL")
    ...
    response_audio_url: Optional[str] = Field(None, description="Output audio file URL")
    processing_ms: int = Field(..., ge=0, description="Processing time in milliseconds")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Session start timestamp")
...
class AdvisoryDeliveryResult(BaseModel):
    ...
    fallback_channel: Optional[DeliveryChannel] = Field(default=None)
...
class SyncOperation(BaseModel):
    ...
    server_version: Optional[int] = Field(default=None, ge=1)
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))

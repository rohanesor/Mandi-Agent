...
from reverie_sdk import ReverieASR, ReverieNMT, ReverieTTS
...
class ReverieVoiceService:
    ...
    async def speech_to_text(self, audio_base64: str, source_language: str) -> str | None:
        if not audio_base64:
            return None
        try:
    ...
    async def translate(self, text: str, source_language: str, target_language: str) -> str | None:
        if not text:
            return None
        ...
        try:
            ...
            response = nmt.localization(data=[text], src_lang=source_language, tgt_lang=target_language, enableNmt=True)
    ...
    async def full_pipeline(
        self, audio_base64: str, farmer_language: str, advisory_english: str, farmer_id: str = "unknown"
    ) -> VoiceSession | None:
...
async def get_voice_service():
    return ReverieVoiceService()


async def close_voice_service():
    pass

...
async def fetch_soil_moisture(
    ...
    try:
        ...
        logger.warning(
            "NASA POWER HTTP %d for (%.2f, %.2f) — using simulated: %s",
            e.response.status_code, lat, lng, str(e)[:100]
        )
    except httpx.TimeoutException:
        logger.warning("NASA POWER timeout for (%.2f, %.2f) — using simulated", lat, lng)
    except httpx.RequestError as e:
        logger.warning("NASA POWER request error for (%.2f, %.2f) — using simulated: %s",
                      lat, lng, str(e)[:100])
    except (KeyError, ValueError) as e:
        logger.warning("NASA POWER parse error for (%.2f, %.2f) — using simulated: %s",
                      lat, lng, str(e)[:100])

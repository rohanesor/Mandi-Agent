...
$failNames = @("Daily Harvest Alerts", "FPO Weekly Digest", "My workflow", "Price Crash Broadcast")
...
foreach ($wf in $workflows) {
    ...
    # Deep-copy nodes and strip credential references
    $cleanNodes = $wf.nodes | ForEach-Object {
        $node = $_ | ConvertTo-Json -Depth 20 | ConvertFrom-Json
        # Remove credentials property from each node
